...
@section('title', __('admin.show_provider'))

@section('content')
    <div class="container-fluid">
        <div class="row justify-content-center">
            <div class="col-12">
                <h2 class="h5 page-title">{{ __('admin.show_provider') }}</h2>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        <x-input id="name" name="name" label="{{ __('admin.name') }}"
                                            placeholder="{{ __('admin.enter_name') }}" :value="$provider->name"
                                            :disabled="true"  />
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <x-select id="country_code" label="{{ __('admin.country_code') }}" name="country_code"
                                        :options="$countries" valueKey="country_code" :value="$provider->country_code" nameKey="name"
                                        :disabled="true" />
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <x-file id="avatar" name="avatar" label="{{ __('admin.enter_image') }}"
                                        :disabled="true" :src="$provider->avatar" />
                                </div>
                            </div>

                            <div class="row">
                                <div class="col-md-6">
                                    <x-checkbox id="is_active" name="is_active" label="{{ __('admin.is_active') }}"
                                    :value="$provider->is_active" :disabled="true" /><br>
                                    <x-checkbox id="is_approved" name="is_approved" label="{{ __('admin.is_approved') }}"
                                        :value="$provider->is_approved" :disabled="true"  /><br>
                                    <x-checkbox id="block_account" name="block_account" label="{{ __('admin.block_account') }}"
                                        :value="$provider->is_blocked" :disabled="true"  />
                                </div>
                            </div>
