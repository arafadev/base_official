                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        <x-input id="name" name="name" label="{{ __('admin.name') }}"
                                            placeholder="{{ __('admin.enter_name') }}" :value="$user->name"  :required="true" />
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <x-select id="country_code" label="{{ __('admin.country_code') }}" name="country_code"
                                        :options="$countries" valueKey="country_code" :value="$user->country_code" nameKey="name" :required="true" />
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <x-file id="avatar" name="avatar" label="{{ __('admin.enter_image') }}"
                                        :required="false" :src="$user->avatar"  />
                                </div>
                            </div>

                            <div class="row">
                                <div class="col-md-6">
                                    <x-checkbox id="is_active" name="is_active" label="{{ __('admin.is_active') }}"  :value="$user->is_active"/><br>
                                    <x-checkbox id="is_notify" name="is_notify" label="{{ __('admin.is_notify') }}"  :value="$user->is_notify"/><br>
                                    <x-checkbox id="is_blocked" name="is_blocked" label="{{ __('admin.block_account') }}"  :value="$user->is_blocked" />
                                </div>
                            </div>
